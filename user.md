...
## Jupyter notebook
### ノートブック機能の概要
Jupter notebookを作成し、KAMONOHASHI上で試すことができます。
Jupyter Labを起動することで、モデル定義コードの開発やデバッグをしたり、トレーニング済みモデルの結果を視覚化したりすることができます。

### ノートブックの作成
ノートブックの起動は[notebook]画面の右上にある[ノートブック作成]ボタンより可能です。
学習の新規実行時と同様にステップごとに下記の必要事項を記入します。

![ノートブック一覧画面](/assets/images/notebook-index.PNG)


|種類|要否|説明|
|---|---|---|
|ノートブック名|必須|ノートブックの名前。<br>半角英数小文字、または記号(“-”（ハイフン）) 30 文字以下で指定可能。|
|CPU|必須|ノートブックで利用するCPUコア数を指定する。|
|メモリ|必須|ノートブックで利用するメモリ量(GB)を指定する。|
|GPU|必須|ノートブックで利用するGPU枚数を指定する。|
|コンテナイメージ|任意|利用するDockerレジストリ、コンテナ、タグを指定する。|
|モデル|任意|ノートブックで利用するソースコードを格納しているGitサーバ、リポジトリ、ブランチ、コミットIDを指定する。コミットIDを指定した場合はブランチの入力値は無視される。コミットIDが未指定の場合はブランチのHEADが選択される。ブランチもコミットIDも未指定の場合は、masterブランチのHEADが選択される。|
|データセット|任意|ノートブックで利用するデータセットを指定する。|
|環境変数|任意|環境変数を指定する。環境変数はコンテナ内の環境変数へ追加設定される。|
|パーティション|任意|KAMONOHASHI に設定されているパーティションを設定する。例えばGPUの型番などはここから指定する。|
|メモ|任意|ノートブック履歴に付与する任意のメモ。|

コンテナイメージを指定しない場合にはデフォルトで下記のイメージが指定されます。
...
ノートブック作成に成功すると一覧画面に作成したノートブックが表示されます。

ノートブック作成直後はステータスが“Pending” となっています。
ステータスが “Running” に変わればノートブックへアクセス可能になります。

![ノートブック作成画面](/assets/images/notebook-running.png)

### ノートブックへのアクセス
ノートブック一覧画面から開きたいノートブックの行内の“ノートブックを開く” ボタンからノートブックへアクセスすることが可能です。

![ノートブック作成画面](/assets/images/notebook-running.png)

JupyterLabのページに遷移します。

![ノートブックアクセス画面](/assets/images/jupyterLab.png)

### ノートブックの停止
停止したいノートブックの行を選択し、ノートブック詳細画面から “停止” ボタンを押下します。

![ノートブック停止画面](/assets/images/notebook-halt.png)

停止に成功するとステータスが “Killed” に変わります。

ノートブックは一定時間使用されていない場合には自動的に停止されます。

### ノートブックの再実行
停止したノートブックは再実行することができます。
再実行したいノートブックの行内の “再実行” ボタンを押下します。
![ノートブック再実行画面](/assets/images/notebook-rerun.png)
必要なリソース量を指定できるようになっているので、入力した後「再実行」を押下します。

![ノートブック再実行画面](/assets/images/notebook-rerun-resource.png)

### ノートブックの削除
停止したいノートブックの行を選択し、ノートブック詳細画面から 左下のごみ箱ボタンを押下します。
![ノートブック削除画面](/assets/images/notebook-halt.png)
削除に成功すると一覧からノートブックが表示されなくなります。
